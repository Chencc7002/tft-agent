...
| 1 Question Contract | 完成 | `question-contract.v2` 已进入 Evidence Pack、LLM 请求、缓存、Validator 与安全诊断 |
...
`src/llm/question-contract.js` 定义 `question-contract.v2`：
...
- `allowedAnswerDimensions`：本问题允许回答的完整维度；包含非必答但有证据时可以回答的维度。
- `requiredEvidence`：逐维度证据要求。
- `requirementContext`：由确定性代码生成的候选数、低样本、对比请求等条件，决定本轮动态必答维度。
...
`src/llm/conclusion-spec-registry.js` 是结论生成层的唯一能力配置入口。每个 `conclusion-spec.v2` 包含：
...
- `conditionalAnswerDimensions`：把数据状态维度声明为 `when_low_sample`、`when_multiple_candidates` 或 `when_comparison_requested`；未声明条件的维度始终必答。
...
装备类结论不再固定要求每次回答全部静态维度。服务端先根据问题类型和已组装证据确定本轮合同：

- 主问题维度始终严格。例如装备对比仍必须回答对比结果和指标；缺一侧数据时必须返回 `insufficient_evidence`，不能伪装成完整结论。
- 仅确有低样本候选时，`sample_risk` 才成为必答维度。稳定样本不需要生成无意义风险套话。
- 仅有至少两个展示候选时，出装排名的 `core_item_tendency` 才成为必答维度。
- 可选维度一旦被回答，仍必须引用该维度要求的合法证据，且继续接受数字、实体、目标和因果边界校验。
- 结构遗漏最多纠错两次；仍不合格则使用确定性模板回退。
...
Validator v4 在严格事实边界前增加确定性 Citation Enricher/Repair：
...
`unit_item_rankings.item_performance` 复用已有单件排名证据，仅新增 Spec 和测试。它在稳定样本下要求：

- `target_item_performance`
- `ranking_context`

仅存在低样本证据时才额外要求 `sample_risk`。

专项测试展示生成的 Question Contract；正确输出覆盖动态必答维度；“证据正确但推荐完整三件套”被 `question_focus_mismatch` 拒绝；目标装备记录缺失时通过合法 `insufficient_evidence` 返回。
